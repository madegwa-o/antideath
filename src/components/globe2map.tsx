...
function interpolateProjection(raw0: () => GeoRawProjection, raw1: () => GeoRawProjection): InterpolatedProjection {
    const mutate = d3.geoProjectionMutator((t: number) => (x: number, y: number): [number, number] => {
    ...
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-expect-error
    projection.alpha = function(value?: number): InterpolatedProjection | number {
...
export function GlobeToMapTransform() {
    ...
    useEffect(() => {
        const loadWorldData = async () => {
            try {
                ...
                const countries = 'features' in featureResult
                    ? featureResult.features as GeoFeature[]
                    : [featureResult] as GeoFeature[]
    ...
    useEffect(() => {
        if (!autoRotate || isDragging || progress[0] !== 0) return
    ...
    }, [autoRotate, isDragging, progress])
